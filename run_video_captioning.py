...
def main(args):
    ...
    if args.dataset == 'msrvtt':
        ...
        assert args.dataset == 'msrvtt' or args.dataset == 'msvd'
    ...
    video_features = torch.load("./feature/" + dataset_name + "/video_features.pkl").to(device)
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(len(video_features))):
                key_flame = video_features[i]
                idx = torch.randperm(key_flame.shape[0])
                key_flame = key_flame[idx, :].view(key_flame.size())
                
                niber = []
                for j in range(len(key_flame)):
                    this_flame = key_flame[j].unsqueeze(0)
                    this_flame = this_flame / this_flame.norm(dim = -1, keepdim = True)
                    similarity = this_flame @ caption_features_norm.T
                    zero = torch.zeros(this_flame.size()).to(device)
                    for k in range(5):
                        _, max_id = torch.max(similarity, dim = 1)
                        zero += caption_features[max_id.item()].unsqueeze(0).to(device)
            ...
            gts = open(f"./output/" + dataset_name + f"/msr_{epoch}.json", "w")
